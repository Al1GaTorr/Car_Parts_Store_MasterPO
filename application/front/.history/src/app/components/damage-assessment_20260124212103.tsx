  ...
  return (
    <div className="min-h-screen bg-zinc-950 pb-24 px-4">
      ...
      <div className="pt-6 pb-4">
        ...
        >
          <h1 className="text-2xl text-white">AI Damage Assessment</h1>
          <p className="text-sm text-zinc-400 mt-1">Scan your vehicle for damage</p>
        </motion.div>
      </div>
      ...
      <motion.div
        ...
        className="relative bg-zinc-900 rounded-2xl overflow-hidden aspect-[3/4] border border-zinc-800 mb-5"
      >
        {/* Simulated Camera Background */}
        <div className="absolute inset-0 bg-gradient-to-b from-zinc-800 to-zinc-900 flex items-center justify-center">
          <Camera size={80} className="text-zinc-700" />
        </div>
        ...
        {!isScanning && !scanComplete && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="text-center">
              <Upload size={48} className="text-zinc-600 mx-auto mb-2" />
              <div className="text-zinc-500 text-sm">Position vehicle in frame</div>
            </div>
          </div>
        )}
      </motion.div>
      ...
      <AnimatePresence>
        {scanComplete && (
          ...
          >
            {/* Status */}
            <div className="bg-gradient-to-br from-red-950/50 to-zinc-900 border border-red-800/30 rounded-2xl p-5">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-white">Damage Detected</h3>
                    <p className="text-xs text-zinc-400">2 areas identified</p>
                  </div>
                </div>
                <button
                  onClick={resetScan}
                ...
                {damageResults.map((damage, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-white text-sm">{damage.area}</span>
                      <span
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-zinc-400">{damage.confidence}%</span>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="bg-zinc-950/50 rounded-xl p-4 border border-zinc-800">
                <div className="text-xs text-zinc-400 mb-2">Estimated Repair Cost</div>
                <div className="flex items-end gap-2">
                  ...
                  <span className="text-zinc-400 mb-1">{estimatedCost.currency}</span>
                </div>
                <div className="text-xs text-zinc-500 mt-1">Based on average market rates</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="bg-zinc-900 border border-zinc-800 text-white py-3 rounded-xl hover:bg-zinc-800 transition-colors"
              >
                Save Report
              </motion.button>
              <motion.button
